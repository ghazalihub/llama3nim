...
proc precomputeFreqsCis*(contextLength, headSize: int, theta: float64,
  ...
  var cr = newSeq[float32](contextLength * headSize)
  var ci = newSeq[float32](contextLength * headSize)
  for pos in 0..<contextLength:
    for i in countup(0, headSize - 1, 2):
      ...
      if ropeScaling:
        ...
        if wavelen < hiFreqWavelen: discard
        elif wavelen > loFreqWavelen: freq = freq / scaleFactor
        else:
...
proc toFloatSeq(ti: GGUFTensorInfo, m: MemFile, dataOffset: int64): seq[float32] =
  let size = ti.dimensions.foldl(a * b, 1'i64).int
  let p = cast[ptr UncheckedArray[float32]](cast[uint](m.mem) + dataOffset.uint + ti.offset.uint)
  result = newSeq[float32](size)
  copyMem(addr result[0], p, size * 4)
...
proc loadModel*(filename: string, contextLength: int): LlamaModel =
  ...
  if metadata.contains("tokenizer.ggml.merges"):
    for line in metadata["tokenizer.ggml.merges"].arr:
      ...
      if parts.len == 2:
        ...
        if f != -1 and s != -1: merges.add((f, s))

  var specialTokens = initTable[string, int]()
  let baseTokens = 128000
  if vocabulary.tokens.len > baseTokens:
    for i in baseTokens..<vocabulary.tokens.len: specialTokens[vocabulary.tokens[i]] = i
  ...
  if metadata.contains("llama.attention.head_count_kv"): config.numberOfKeyValueHeads = metadata["llama.attention.head_count_kv"].getInt()
  else: config.numberOfKeyValueHeads = config.numberOfHeads

  if contextLength > 0: config.contextLength = contextLength
  ...
  if ti.contains("output.weight"): weights.wcls = loadQuantized(ti["output.weight"], m, doff)
  else: weights.wcls = weights.tokenEmbeddingTable

  let (cr, ci) = precomputeFreqsCis(config.contextLength, config.headSize, config.ropeTheta,
                                   ti.contains("rope_freqs"), 8.0f32, 1.0f32, 3.0f32, 8192.0f32)
